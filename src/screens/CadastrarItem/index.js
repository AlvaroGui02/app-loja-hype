...
export default function CadastrarVeiculo({ navigation }) {
    ...
    const validar = () => {
        if (item == "") {
            setErrorCadastrarVenda("Informe o item")
        } else if (preco == "") {
            setErrorCadastrarVenda("Informe o preço")
        } else {
    ...
    const cadastrarVenda = () => {
        const taskListRef = dbRef(db, 'loja/' + auth.currentUser.uid);
        ...
        set(newTaskRef, {
            ...
            .then(() => {
                navigation.navigate('Tabs', { screen: 'Lojas Hype' });
            })
            .catch((error) => {
                console.error('Erro ao cadastrar item:', error);
                setErrorCadastrarVenda('Erro ao cadastrar item. Por favor, tente novamente.');
            });
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.button} onPress={validar}>
                <Text style={styles.textButton}>Cradastrar</Text>
            </TouchableOpacity>
        </View>
    )
}